  ...
  const handleSort = (key: string) => {
    ...
    const sortedOrders = [...orders].sort((a, b) => {
      if (key === "total") {
        ...
        return direction === "asc"
          ? String(a[key as keyof Order]).localeCompare(
              String(b[key as keyof Order])
            )
          : String(b[key as keyof Order]).localeCompare(
              String(a[key as keyof Order])
            );
      }
    });
        ...
        <table className="w-full border-collapse shadow-lg rounded-lg overflow-hidden">
          <thead className="bg-gray-700 text-white">
            <tr className="text-center">
              ...
              <th className="px-4 py-3 cursor-pointer" onClick={() => handleSort("created_at")}> Tạo lúc <FaSort className="inline" />
              </th>
              <th className="px-4 py-3 cursor-pointer" onClick={() => handleSort("updated_at")}> Cập nhật lúc <FaSort className="inline" />
              </th>
                ...
                <tr key={order.id} className="text-center border-b border-gray-700 hover:bg-gray-700">
                  ...
                  <td className="px-4 py-3">
                    {editingOrderId === order.id ? (
                      (() => {
                        const statusFlow: Order["status"][] = ["pending", "processing", "shipping", "delivered",];
                        const currentStatusIndex = statusFlow.indexOf( order.status);
                        const availableStatusOptions = statusFlow.slice(currentStatusIndex);
                        return (
                          <select title="Status" value={newStatus} onChange={(e) =>
                  ...
                  <td className="px-4 py-3">
                    {order.updated_at ? new Date(order.updated_at).toLocaleString("vi-VN") : "Chưa cập nhật"}
                  </td>
                  <td className="px-4 py-3 flex justify-center items-center gap-2 flex-wrap">
                    {editingOrderId === order.id ? (
                      <button onClick={handleSaveStatus} className="bg-emerald-500 px-3 py-2 rounded-md hover:bg-emerald-600 transition-all flex items-center gap-1 text-sm">
                        <FaSave /> Lưu
                      </button>
                    ) : (
                      order.status !== "cancelled" && order.status !== "delivered" && (
                        <button onClick={() =>
                            handleStartEdit(order.id, order.status)
                          }
